...
class GINLayer(nn.Module):
    ...
    def forward(self, g, h, e):
        ...
        g.edata['ew'] = self.bond_encoder(e)
        g = g.local_var()
        g.ndata['h'] = h
        #g.update_all(fn.copy_u('h', 'm'), self._reducer('m', 'neigh'))

        g.update_all(edge_msg, self._reducer('m', 'neigh'))
...
class MLP(nn.Module):
    ...
    def __init__(self, num_layers, assign_num, input_dim, hidden_dim, output_dim):
        ...
        if num_layers < 1:
            ...
            for layer in range(num_layers - 1):
                self.batch_norms.append(nn.BatchNorm1d((assign_num)))
