...
class DiffPoolNet(nn.Module):
    ...
    def __init__(self, net_params):
        ...
        self.pre_features = Variable(torch.zeros((self.batch_size, self.assign_num * hidden_dim)).cuda())
        self.pre_weights =  Variable(torch.ones((self.batch_size, 1)).cuda())
        ...
        for _ in range(n_layers - 1):
            gc_after_per_pool.append(DenseGraphSage(hidden_dim, hidden_dim, self.residual, assign_num=self.assign_num))
        gc_after_per_pool.append(DenseGraphSage(hidden_dim, embedding_dim, self.residual, assign_num=self.assign_num))
