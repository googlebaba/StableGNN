...
from layers.gin_layer import GINLayer   # this is GraphSageLayer
...
from layers.tensorized.dense_gin_layer import DenseGIN
...
class DiffPoolNet(nn.Module):
    ...
    def __init__(self, net_params):
        ...
        mlp = MLP(2, hidden_dim, hidden_dim, hidden_dim, hidden_dim)
        self.gc_before_pool.append(GINLayer(ApplyNodeFunc(mlp), 'sum', dropout, self.batch_norm, self.residual, init_eps=0, learn_eps=True))
        ...
        self.first_diffpool_layer = DiffPoolLayer(pool_embedding_dim, self.assign_dim, hidden_dim,activation, dropout, aggregator_type, self.link_pred)
        ...
        for _ in range(n_layers - 1):
            ...
            mlp = MLP(2, self.assign_num, hidden_dim, hidden_dim, hidden_dim)
            gc_after_per_pool.append(DenseGIN(hidden_dim, hidden_dim, ApplyNodeFunc(mlp), dropout, self.residual))

        mlp = MLP(2, self.assign_num, hidden_dim, hidden_dim, hidden_dim)
        gc_after_per_pool.append(DenseGIN(hidden_dim, embedding_dim, ApplyNodeFunc(mlp), dropout, self.residual, last_layer = True))
